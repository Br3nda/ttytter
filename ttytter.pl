...
$TTYtter_VERSION = '0.6b'; #the b stands for Br3nda
...
$timestamp ||= 0;
#default timestamp format, if not enable but format not specified
$timestamp = "%Y-%m-%d %k:%M:%S" if ($timestamp eq "1");
...
sub defaulthandle {
  my $ref = shift;
  my $speaker =  &descape($ref->{'user'}->{'screen_name'});
  my $tweet = &descape($ref->{'text'});
  my $g = "<$speaker> $tweet\n";

  if ($timestamp) {
    my $time = str2time($ref->{'created_at'});
    my $ts = time2str($timestamp, $time);
    $g = "$ts $g";
  } 
  ...
  my $username = $temp[0];
  #print the tweet
  if ($speaker eq $username) {
    #if it's me speaking, colour the line blue
    print BOLD, BLUE, "$g", RESET;
  } elsif ($tweet =~ /$username/i) {
    #if my name is in the reply, colour the line red
    print BOLD, RED, "$g", RESET;
  } else {
